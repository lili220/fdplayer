 ...
 * $Id: 23c336dc5d321402362ac0af4d364f5234415e1a $
 *
...
/*add by lili*/
#if 0
void DialogsProvider::userDialog()
{
	if( UserDialog::getInstance( p_intf )->isLogin() )
	{
		// todo Message box You have login already
		UserShareDialog::getInstance( p_intf )->toggleVisible();
	}
	else
	{
		UserDialog::getInstance( p_intf )->toggleVisible();
	}
}
#endif

#include <QMessageBox>
void DialogsProvider::loginDialog()
{
	if( LoginDialog::getInstance( p_intf )->isLogin() )
	{
		QString user = LoginDialog::getInstance( p_intf )->getUsername();
		QString infoText = user.append( qtr( "已登录，如需更改用户请先退出！" ) );
...
void DialogsProvider::registorDialog()
{
	RegistorDialog::getInstance( p_intf )->toggleVisible();
}

void DialogsProvider::userLogout()
{
	LoginDialog::getInstance( p_intf )->logout();
}
