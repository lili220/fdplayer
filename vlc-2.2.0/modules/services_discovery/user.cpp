...
int User::nfschina_registor( QString username, QString password )
{
	pArgs = PyTuple_New( 2 );
...
//int User::nfschina_login( QString username, QString password, bool b_share, QString ip, QString port )
int User::nfschina_login( QString username, QString password )
{
...
int User::nfschina_logout( int userid )
{
...
int User::nfschina_keeponline( int userid, bool b_share )
{
	printf( "------------------------%s-------------------------\n", __func__ );
...
int User::nfschina_upLoad( int userid, QString filename, QString filepath )
{
	if( !isLogin() )
	{
...
void User::nfschina_listMyFile( int userid )
{
	if( !isLogin() )
	{
		QMessageBox msgBox( QMessageBox::Information,
...
int User::nfschina_delete( int userid, QString filename )
{
	if( !isLogin() )
	{
		QMessageBox msgBox( QMessageBox::Information,
...
QString User::nfschina_download( int userid, QString filename )
{
	if( !isLogin() )
	{
		QMessageBox msgBox( QMessageBox::Information,
...
#if 0
void User::toggleLocalShared( bool state )
{
	printf( "----------------------%s-----------------------\n", __func__ );
...
void User::toggleNetShared( bool state )
{
	printf( "----------------------%s-----------------------\n", __func__ );
...
void User::nfschina_keepalive(QString localIp, int localPort, int userid, bool is_share, QString ServerUrl)
{
	printf( "----------------------%s-----------------------\n", __func__ );
#if 0
